/* comtest.c : Capstone CAN Shifter
	...
	for( ;; ) {
		...
		for( cByteToSend = comFIRST_BYTE; cByteToSend <= comLAST_BYTE; cByteToSend++ ) {
			if( xSerialPutChar( xPort, cByteToSend, comNO_BLOCK ) == pdPASS ) {
				vLedHelperToggleLED( uxBaseLED + comTX_LED_OFFSET );
			}
		}

		/* Turn the LED off while we are not doing anything. */
		vLedHelperSetLED( uxBaseLED + comTX_LED_OFFSET, pdFALSE );
...
BaseType_t xResyncRequired = pdFALSE, xErrorOccurred = pdFALSE;
	...
	for( ;; ) {
		...
		for( cExpectedByte = comFIRST_BYTE; cExpectedByte <= comLAST_BYTE; cExpectedByte++ ) {
			...
			if( xSerialGetChar( xPort, &cByteRxed, comRX_BLOCK_TIME ) ) {
				...
				if( cByteRxed == cExpectedByte ) {
					vLedHelperToggleLED( uxBaseLED + comRX_LED_OFFSET );
				} else {
		...
		/* Turn the LED off while we are not doing anything. */
		vLedHelperSetLED( uxBaseLED + comRX_LED_OFFSET, pdFALSE );
